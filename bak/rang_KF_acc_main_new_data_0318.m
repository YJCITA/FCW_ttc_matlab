...
for l1_i = 1 : length( sub_l1_dir )
    ...
    for l2_i = 1 : length( sub_l2_dir )
        ...
        %         mydate = regexp(str,expression,'names')
                ...
                if(str_frame_index == 0)
                  ...
                  %% FCW触发逻辑判断
                    ...
                    % vel < -1
                    % 相对车距离<20 ?? 这个限制应该来说是不合理 可以考虑限制一个大点的距离 防止误报
                    range_cur = Xk(1);
                    vel_cur = Xk(2);
                    acc_cur = Xk(3);
                    if(ttc<4 && ttc>0 && speed_cur>40/3.6 && abs(vision_horizon)<1.2 && vel_cur<-1 && range_cur<35 )
        ...
        %     figure('visible','off')    
            ...
            plot(save_vision_horizon(1,:), save_vision_horizon(2,:)*10, '.'); % vision horizon
            plot(save_speed_car(1,:), save_speed_car(2,:), '.'); % speed-car
            grid on;
            legend({'vel-estimation','acc-estimation', 'vision-horizon*10', 'speed-car'},'Location','northeast','FontSize',10);
%             ylim([-30, 10]);
            legend('boxoff')
            ...
            plot(save_ttc(1,:), ones(1,NUM)*-4)
            grid on;
            legend({'ttc', 'fcw-mobileye','fcw-minieye', 'ttc = 4', 'ttc = -4'},'Location','northeast','FontSize',10);
            legend('boxoff')
