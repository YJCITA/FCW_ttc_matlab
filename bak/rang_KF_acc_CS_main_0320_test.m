...
%% 2017.03.20 CA模型对于波动情况难以解决，会有误报，继续考虑CS模型
% 有点问题 需要再继续系统调试（考虑是否用ADRC跟踪会有效果）
clc
clear all
close all
...
%% 数据导入
% 遍历文件夹
maindir = 'F:\数据\FCW\BACK\0316\跟踪的速度\20';
...
%                         [Xk, Pk] = fun_KF(Xk, Pk, z, Q, R, F, H);
                     ...
                     %%   CS模型
                        ...
                        a_max = [-15, 15];
                        O33 = zeros(3, 3);
                        F_sub = [ 1  T  (-1+alpha*T+exp(-alpha*T))/(alpha^2);  % 一个维度
                        ...
                        U_sub = [ (-T + alpha*T^2 + ( 1-exp(-alpha*T) )/alpha )/ alpha;  % 一个维度
